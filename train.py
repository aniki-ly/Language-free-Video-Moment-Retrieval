...
import argparse
os.environ["WANDB_API_KEY"] = ' In Put Your Wandb Key Here '
...
parser.add_argument("--wandb", '-w', type=str,default='No')
...
if args.wandb:
    import wandb
    wandb.init(project="LFVMR", entity=" Your ID Name ", name=args.wandb)
    wandb.run.log_code(".")
#%%
...
for epoch in pbar:
    ...
    for batch_idx,item in enumerate(dataloaders['train']):
        ...
        pbar.set_description("training {}/{}, Loss:{:.4f}, LR: {:.6f}".format(batch_idx,len(dataloaders['train']), sum_loss.item(), optimizer.param_groups[0]['lr']))
