...
class CharadesBasicDataset(Dataset):
    def __init__(self, cfg, vid_path, split, wtoi, itow, annos):
        ...
        self.itow = itow
        self.annos = annos
        # self.feats, self.clip_feats = self._load_vid_feats()
    ...
    def _load_vid_feats(self):
        ...
        for vid in tqdm(vid_list, desc="loading video features"):
            ...
            clip_feats[vid] = np.load(join('charades_clip_features',"{}.npy".format(vid))).squeeze()
        return feats, clip_feats
    ...
    def get_fixed_length_feat(self, feat, num_segment, start_pos, end_pos):
        ...
        if end_index is None:
            end_index = min(nfeats, num_segment)+1
    ...
    def __getitem__(self,idx):
        ...
        vid_feat = np.load(join(self.vid_path,"{}.npy".format(vid))).squeeze()
        ...
        if self.split != "train":
            clip_text_feat = torch.load(f'test_text_feats/{vid}_{timestamp[0]}_{timestamp[1]}.npy') # 512
            instance.update({"clip_text_feat": torch.FloatTensor(clip_text_feat).unsqueeze(0)})


        if self.split == 'train':
            # clip_vid_feat = self.clip_feats[vid]
            clip_vid_feat = np.load(join('charades_clip_features',"{}.npy".format(vid))).squeeze()
            fixed_vid_feat_clip, nfeats_clip, start_index_clip, end_index_clip = self.get_fixed_length_feat(clip_vid_feat, self.num_segments, start_pos, end_pos)
            fixed_vid_feat_clip = fixed_vid_feat_clip[start_index_clip:end_index_clip-1]
            # random sample 9 clips
            if len(fixed_vid_feat_clip) > 9:
                fixed_vid_feat_clip = fixed_vid_feat_clip[np.random.choice(len(fixed_vid_feat_clip),9,replace=False)]
            else:
                new_indices = np.linspace(0,len(fixed_vid_feat_clip)-1,9).astype(int)
                fixed_vid_feat_clip = fixed_vid_feat_clip[new_indices]
            
            clip_dict = {
                "clip_vid_feats": torch.FloatTensor(fixed_vid_feat_clip),
            }
            instance.update(clip_dict)
        return instance

    def collate_fn(self, data):
        seq_items = ["video_feats", "video_masks","attention_masks", "clip_vid_feats"]
        tensor_items = [
            "query_labels", "query_masks", "nfeats",
            "grounding_start_pos", "grounding_end_pos", "clip_text_feat"
        ]
